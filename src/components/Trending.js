import { useProductContext } from "../context/productcontex";
import styled from "styled-components";
import Product from "./Product";
import styles from './Trending.module.css'; 

const Trending = () => {
  const { isLoading, featureProducts } = useProductContext();

  if (isLoading) {
    return <div style={{textAlign: "center", backgroundColor: "black"}}> <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExcjAzM2VoenNoODI5aGczdXMzM3B1dWU1M3pzeDJjcGd5MjFsOTJ3YSZlcD12MV9naWZzX3NlYXJjaCZjdD1n/uIJBFZoOaifHf52MER/giphy.gif"/> </div>;
  }

  return (
  
    <section className={styles.game_section}>
      <h2 className={styles.line_title}>trending games</h2>
      <div className={[styles.owl_theme_custom_carousel ,styles.owl_dots]}>
        <div className={[styles.item, styles.active]} style={{backgroundImage: 'url(https://www.yudiz.com/codepen/expandable-animated-card-slider/dota-2.jpg)'}}>
          <div className={styles.item_desc}>
            <h3>Dota 2</h3>
            <p>Dota 2 is a multiplayer online battle arena by Valve. The game is a sequel to Defense of the
              Ancients, which was a community-created mod for Blizzard Entertainment's Warcraft III.</p>
          </div>
        </div>
        <div className={styles.item} style={{backgroundImage: 'url(https://www.yudiz.com/codepen/expandable-animated-card-slider/winter-3.jpg)'}}>
          <div className={styles.item_desc}>
            <h3>The Witcher 3</h3>
            <p>The Witcher 3 is a multiplayer online battle arena by Valve. The game is a sequel to Defense
              of the Ancients, which was a community-created mod for Blizzard Entertainment's Warcraft III.</p>
          </div>
        </div>
        <div className={styles.item} style={{backgroundImage: 'url(https://www.yudiz.com/codepen/expandable-animated-card-slider/rdr-2.jpg)'}}>
          <div className={styles.item_desc}>
            <h3>RDR 2</h3>
            <p>RDR 2 is a multiplayer online battle arena by Valve. The game is a sequel to Defense of the
              Ancients, which was a community-created mod for Blizzard Entertainment's Warcraft III.</p>
          </div>
        </div>
        <div className={styles.item} style={{backgroundImage: 'url(https://www.yudiz.com/codepen/expandable-animated-card-slider/pubg.jpg)'}}>
          <div className={styles.item_desc}>
            <h3>PUBG Mobile</h3>
            <p>PUBG 2 is a multiplayer online battle arena by Valve. The game is a sequel to Defense of the
              Ancients, which was a community-created mod for Blizzard Entertainment's Warcraft III.</p>
          </div>
        </div>
        <div className={styles.item} style={{backgroundImage: 'url(https://www.yudiz.com/codepen/expandable-animated-card-slider/fortnite.jpg)'}}>
          <div className={styles.item_desc}>
            <h3>Fortnite</h3>
            <p>Battle royale where 100 players fight to be the last person standing. which was a community-created mod
              for Blizzard Entertainment's Warcraft III.</p>
          </div>
        </div>
        <div className={styles.item} style={{backgroundImage: 'url(https://www.yudiz.com/codepen/expandable-animated-card-slider/far-cry-5.jpg)'}}>
          <div className={styles.item_desc}>
            <h3>Far Cry 5</h3>
            <p>Far Cry 5 is a 2018 first-person shooter game developed by Ubisoft.
              which was a community-created mod for Blizzard Entertainment's Warcraft III.</p>
          </div>
        </div>
      </div>
    </section>
  );
};



const Wrapper = styled.section`
  padding: 9rem 0;
  background-color: ${({ theme }) => theme.colors.bg};

  .container {
    max-width: 120rem;
  }

  figure {
    width: auto;
    display: flex;
    justify-content: center;
    align-items: center;
    position: relative;
    overflow: hidden;
    transition: all 0.5s linear;
    &::after {
      content: "";
      position: absolute;
      top: 0;
      left: 0;
      width: 0%;
      height: 100%;
      background-color: rgba(0, 0, 0, 0.5);
      transition: all 0.2s linear;
      cursor: pointer;
    }
    &:hover::after {
      width: 100%;
    }
    &:hover img {
      transform: scale(1.2);
    }
    img {
      max-width: 90%;
      margin-top: 1.5rem;
      height: 20rem;
      transition: all 0.2s linear;
    }

    .caption {
      position: absolute;
      top: 15%;
      right: 10%;
      text-transform: uppercase;
      background-color: ${({ theme }) => theme.colors.navbg};
      color: ${({ theme }) => theme.colors.red};
      padding: 0.8rem 2rem;
      font-size: 1.2rem;
      border-radius: 2rem;
    }
  }

  .card {
    background-color: ${({ theme }) => theme.colors.navbg};
    border-radius: 2rem;
    padding-bottom: 0.2rem;
    padding-top: 1rem;

    .card-data {
      padding: 0 2rem;
    }

    .card-data-flex {
      margin: 2rem 0;
      display: flex;
      justify-content: space-between;
      align-items: center;
    }

    h3 {
      color: white;
      text-transform: capitalize;
    }

    .card-data--price {
      color: ${({ theme }) => theme.colors.red};
    }

    .btn {
      margin: 2rem auto;
      background-color: rgb(0 0 0 / 0%);
      border: 0.1rem solid rgb(98 84 243);
      display: flex;
      justify-content: center;
      align-items: center;

      &:hover {
        background-color: rgb(98 84 243);
      }

      &:hover a {
        color: #fff;
      }
      a {
        color: rgb(98 84 243);
        font-size: 1.4rem;
      }
    }
  }
`;

export default Trending;
